    ...
    char*& buffer, GemmIdType const& gemmId) const
{
    ...
    tensorrt_llm::plugins::write(buffer, static_cast<int>(mProfileMap->size()));
    for (auto const& pair : *mProfileMap)
    {
        // Save pair of M to the best GEMM config
        tensorrt_llm::plugins::write(buffer, pair);
    }
}
    ...
    char const*& data, GemmDims& dims, GemmIdType const& gemmId)
{
    ...
    int selectedMapSize;
    tensorrt_llm::plugins::read(data, selectedMapSize);
    for (int ii = 0; ii < selectedMapSize; ++ii)
    {
        std::pair<int, std::optional<Config>> config;
        tensorrt_llm::plugins::read(data, config);
    ...
    RunnerPtr const& runner, nvinfer1::DataType const& type, GemmDims const& dims, GemmIdType const& gemmId)
{
    ...
    tensorrt_llm::common::check_cuda_error(cudaStreamCreate(&mStream));
    ...
    // Free tmp data
    freeTmpData();
    tensorrt_llm::common::check_cuda_error(cudaStreamDestroy(mStream));
}
