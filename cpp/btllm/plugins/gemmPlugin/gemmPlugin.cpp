...
using btllm::plugins::GemmDims;
using btllm::plugins::GemmPluginCreator;
using btllm::plugins::GemmPlugin;
using btllm::plugins::CublasLtGemmPluginProfiler;
using btllm::plugins::CublasGemmWrapperPtr;
...
void GemmPlugin::configurePlugin(size_t minM, size_t maxM, size_t N, size_t K) noexcept
{//@# give A is row major, if mTransA, then M is col of A, otherwise is row of A
...
size_t GemmPlugin::getWorkspaceSize() const noexcept
{
    return CUBLAS_WORKSPACE_SIZE;
}

int GemmPlugin::enqueue(size_t rowA, size_t colA, size_t rowB, size_t colB, void const* A, void const* B, void* outputs, void* workspace, cudaStream_t stream) noexcept
{
    // inputs
    ...
    int const nbDimsA = 2; //origin: inputDesc[0].dims.nbDims;
    ...
    auto const M = ( mTransA ? colA : rowA) - padM;//@# give A is row major, if mTransA, then M is col of A, otherwise is row of A
    // auto const N = utils::computeNDimension(mTransB, inputDesc[1].dims) - padN;
    auto const N = (mTransB ? rowB : colB ) - padN;//@# give B is row major, if mTransB, then N is row of B, otherwise is col of B
    // int const K = static_cast<utils::DimType64>(
    //     mTransA ? inputDesc[0].dims.d[0] - padK : inputDesc[0].dims.d[nbDimsA - 1] - padK);
    int const K = ( mTransA ? rowA : colA) - padK;//@# give A is row major, if mTransA, then K is row of A, otherwise is col of A

    auto bestTactic = mPluginProfiler->getBestConfig(M, mGemmId);
    // runGemm(M, N, K, mTransA, mTransB, mPadLda, mPadLdb, mType, mCublasWrapper, A, B, outputs,
    //     bestTactic, workspace, stream);
        ...
        int lda, ldb, ldc;
        // getProblemParams(transa, transb, m, n, k, lda, ldb, ldc, mTransA, mTransB, M, N, K, mPadLda, mPadLdb);
            transa = mTransB ? CUBLAS_OP_T : CUBLAS_OP_N;
            transb = mTransA ? CUBLAS_OP_T : CUBLAS_OP_N;
            m = N;
            n = M;
            k = K;
            lda = mTransB ? K + mPadLdb : N + mPadLdb;
            ldb = mTransA ? M + mPadLda : K + mPadLda;
            ldc = N;

        mCublasWrapper->createDescriptors(transa, transb, m, n, k, lda, ldb, ldc);
        mCublasWrapper->Gemm(transa, transb, m, n, k, B/* weight */, lda, A/* act */, ldb, outputs, ldc, bestTactic/* heuristic */);
...
GemmPlugin* GemmPluginCreator::createPlugin(size_t minM, size_t maxM, size_t N, size_t K, int transA, int transB, int padLda, int padLdb, nvinfer1::DataType type, bool useFp8) noexcept
{
    ...
    try
    {
        ...
        auto* obj = new btllm::plugins::GemmPlugin(transA, transB, padLda, padLdb, type, useFp8, pluginProfiler);
        // obj->setPluginNamespace(mNamespace.c_str());
        obj->configurePlugin(minM, maxM, N, K);
        obj->initialize(); 
        return obj;
    }
    catch (std::exception const& e)
