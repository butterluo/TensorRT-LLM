 ...
 */
#include "btweightOnlyGroupwiseQuantMatmulPlugin.h"
...
using btllm::plugins::BTWeightOnlyGroupwiseQuantGemmPluginProfiler;
...
using tensorrt_llm::plugins::FP16_INT4_RATIO;
...
// PluginFieldCollection BTWeightOnlyGroupwiseQuantMatmulPluginCreator::mFC{};
// std::vector<nvinfer1::PluginField> BTWeightOnlyGroupwiseQuantMatmulPluginCreator::mPluginAttributes;

void BTWeightOnlyGroupwiseQuantGemmPluginProfiler::runTactic(int m, int n, int k,
    BTWeightOnlyGroupwiseQuantGemmPluginProfiler::Config const& tactic, char* workspace, cudaStream_t const& stream)
{
    // Quantized weights are packed in FP16 format (INT4*4 -> FP16)
    int const originalN = n * tensorrt_llm::plugins::FP16_INT4_RATIO;
...
void BTWeightOnlyGroupwiseQuantGemmPluginProfiler::computeTmpSize(int maxM, int n, int k)
{
    // Quantized weights are packed in FP16 format (INT4*4 -> FP16)
    int const originalN = n * tensorrt_llm::plugins::FP16_INT4_RATIO;
...
std::vector<BTWeightOnlyGroupwiseQuantGemmPluginProfiler::Config> BTWeightOnlyGroupwiseQuantGemmPluginProfiler::getTactics(
    int m, int n, int k) const
{
    return mRunner->getConfigs();
}

BTWeightOnlyGroupwiseQuantMatmulPlugin::BTWeightOnlyGroupwiseQuantMatmulPlugin(nvinfer1::DataType type, int quant_algo,
    int group_size, BTWeightOnlyGroupwiseQuantMatmulPlugin::PluginProfilerPtr const& pluginProfiler)
...
// BTWeightOnlyGroupwiseQuantMatmulPlugin::BTWeightOnlyGroupwiseQuantMatmulPlugin(
//     void const* data, size_t length, BTWeightOnlyGroupwiseQuantMatmulPlugin::PluginProfilerPtr const& pluginProfiler)
//     : mPluginProfiler(pluginProfiler)
// {
...
void BTWeightOnlyGroupwiseQuantMatmulPlugin::init(nvinfer1::DataType type, int quant_algo, int group_size)
{
...
//     }
// #if defined(ENABLE_BF16) //@#TODO only support BF16 NOT FP16
    if (mType == nvinfer1::DataType::kBF16)
    {
        ...
        //     if (quant_algo & ZERO)
        //     {
                assert(quant_algo & ZERO); //@#TODO only support '0点也偏移'
...
// nvinfer1::IPluginV2DynamicExt* BTWeightOnlyGroupwiseQuantMatmulPlugin::clone() const noexcept
// {
//     auto* plugin = new BTWeightOnlyGroupwiseQuantMatmulPlugin(*this);
//     return plugin;
// }

void BTWeightOnlyGroupwiseQuantMatmulPlugin::configGemm()
{
    mPluginProfiler->profileTactics(m_weightOnlyGroupwiseGemmRunner, mType, mDims, mGemmId);
}

// nvinfer1::DimsExprs BTWeightOnlyGroupwiseQuantMatmulPlugin::getOutputDimensions(
...
// }
//
// bool BTWeightOnlyGroupwiseQuantMatmulPlugin::supportsFormatCombination(
...
// void BTWeightOnlyGroupwiseQuantMatmulPlugin::configurePlugin(nvinfer1::DynamicPluginTensorDesc const* in, int nbInputs,
//     nvinfer1::DynamicPluginTensorDesc const* out, int nbOutputs) noexcept
void BTWeightOnlyGroupwiseQuantMatmulPlugin::configurePlugin(const int maxM, const int maxN, const int maxK) noexcept
{
    ...
    auto const minM = maxM;
    // int const maxK = in[0].max.d[in[0].max.nbDims - 1];
    //
    // Quantized weights are packed in FP16 format (INT4*4 -> FP16)
    // int const maxN = in[mWeightInputIdx].max.d[1] * tensorrt_llm::plugins::FP16_INT4_RATIO;
    assert( (maxN % FP16_INT4_RATIO) == 0 );
    auto const K = maxK;
    ...
    size_t smoothedActSize = static_cast<size_t>(maxM) * static_cast<size_t>(maxK)
        * sizeof(half); //(in[0].desc.type == nvinfer1::DataType::kFLOAT ? sizeof(float) : sizeof(half)); support BF16 only
    m_workspaceMaxSize = smoothedActSize + m_weightOnlyGroupwiseGemmRunner->getWorkspaceSize(maxM, maxN, maxK);
}

size_t BTWeightOnlyGroupwiseQuantMatmulPlugin::getWorkspaceSize() const noexcept
{
    return m_workspaceMaxSize;
}

// int BTWeightOnlyGroupwiseQuantMatmulPlugin::enqueue(nvinfer1::PluginTensorDesc const* inputDesc,
//     nvinfer1::PluginTensorDesc const* outputDesc, void const* const* inputs, void* const* outputs, void* workspace,
//     cudaStream_t stream) noexcept
int BTWeightOnlyGroupwiseQuantMatmulPlugin::enqueue(int const m, int const real_n, int const k, 
          ...
          void* workspace, cudaStream_t stream) noexcept
{
    ...
    // }
    // int const m = TLLM_INT32_CAST(m64);                                                     //@#IO
    // int const n = TLLM_INT32_CAST(inputDesc[mWeightInputIdx].dims.d[1]);                    //@#IO use real_n instead. n = (real_n + FP16_INT4_RATIO)/FP16_INT4_RATIO
    // int const k = TLLM_INT32_CAST(inputDesc[0].dims.d[inputDesc[0].dims.nbDims - 1]);       //@#IO
    assert(real_n % FP16_INT4_RATIO == 0);
    int const n = real_n/FP16_INT4_RATIO; // (real_n + FP16_INT4_RATIO - 1)/FP16_INT4_RATIO;

    bool use_cuda_kernel = m < SMALL_M_FAST_PATH && mCudaKernelEnabled;
    bool use_pre_quant_scale = mQuantAlgo & PRE_QUANT_SCALE;

    // half const* zeros_ptr = (mQuantAlgo & ZERO) ? reinterpret_cast<half const*>(inputs[mZerosInputIdx]) : nullptr;       //@#IO
    // half const* biases_ptr = (mQuantAlgo & BIAS) ? reinterpret_cast<half const*>(inputs[mBiasesInputIdx]) : nullptr;     //@#IO
    // half const* act_ptr = reinterpret_cast<half const*>(inputs[0]);                                                      //@#IO
    float alpha = 1.0;
...
//     }
//
//     if (use_pre_quant_scale && !use_cuda_kernel)  //@# NOT support pre_quant (act quant)
//     {
...
//     }
    ...
    // Quantized weights are packed in FP16 format (INT4*4 -> FP16)
    // int real_n = n * tensorrt_llm::plugins::FP16_INT4_RATIO;
    // if (use_cuda_kernel)               //@#TODO use cutlass temporarily
    // {
    ...
    // {
        ...
        // int32_t* weight_ptr = const_cast<int32_t*>(reinterpret_cast<int32_t const*>(inputs[mWeightInputIdx]));     //@#IO
        ...
        TLLM_CHECK_WITH_INFO(bestTactic,
            ...
            "the "
            "engine.)");
        m_weightOnlyGroupwiseGemmRunner->gemm(act_ptr, weight_ptr, weight_scales_ptr/*inputs[mScalesInputIdx]*/, zeros_ptr, biases_ptr,
            alpha, output_ptr/* outputs[0] */, m, real_n, k, mGroupSize, *bestTactic,
...
// nvinfer1::DataType BTWeightOnlyGroupwiseQuantMatmulPlugin::getOutputDataType(
...
// IPluginV2 Methods
//
// char const* BTWeightOnlyGroupwiseQuantMatmulPlugin::getPluginType() const noexcept
// {
//     return WOQ_GROUPWISE_MATMUL_PLUGIN_NAME;
// }
//
// char const* BTWeightOnlyGroupwiseQuantMatmulPlugin::getPluginVersion() const noexcept
// {
//     return WOQ_GROUPWISE_MATMUL_PLUGIN_VERSION;
// }
//
// int BTWeightOnlyGroupwiseQuantMatmulPlugin::getNbOutputs() const noexcept
// {
//     return 1;
// }

int BTWeightOnlyGroupwiseQuantMatmulPlugin::initialize() noexcept
{
    configGemm();
    return 0;
}

// void BTWeightOnlyGroupwiseQuantMatmulPlugin::terminate() noexcept {}
//
// size_t BTWeightOnlyGroupwiseQuantMatmulPlugin::getSerializationSize() const noexcept
// {
...
// }
//
// void BTWeightOnlyGroupwiseQuantMatmulPlugin::serialize(void* buffer) const noexcept
// {
...
// }
//
// void BTWeightOnlyGroupwiseQuantMatmulPlugin::destroy() noexcept
// {
...
BTWeightOnlyGroupwiseQuantMatmulPluginCreator::BTWeightOnlyGroupwiseQuantMatmulPluginCreator()
{
...
// char const* BTWeightOnlyGroupwiseQuantMatmulPluginCreator::getPluginName() const noexcept
// {
//     return WOQ_GROUPWISE_MATMUL_PLUGIN_NAME;
// }
//
// char const* BTWeightOnlyGroupwiseQuantMatmulPluginCreator::getPluginVersion() const noexcept
// {
//     return WOQ_GROUPWISE_MATMUL_PLUGIN_VERSION;
// }
//
// PluginFieldCollection const* BTWeightOnlyGroupwiseQuantMatmulPluginCreator::getFieldNames() noexcept
// {
//     return &mFC;
// }

// IPluginV2* BTWeightOnlyGroupwiseQuantMatmulPluginCreator::createPlugin(
//     char const* name, PluginFieldCollection const* fc) noexcept
BTWeightOnlyGroupwiseQuantMatmulPlugin* BTWeightOnlyGroupwiseQuantMatmulPluginCreator::createPlugin(char const* name, nvinfer1::DataType type, int quant_algo, int group_size,
  const int maxM, const int maxN, const int maxK) noexcept
{
    ...
    try
    {
        // BTWeightOnlyGroupwiseQuantMatmulPluginCreator is unique and shared for an engine generation
        ...
        BTWeightOnlyGroupwiseQuantMatmulPlugin* obj = new BTWeightOnlyGroupwiseQuantMatmulPlugin(type, quant_algo, group_size, pluginProfiler);
        //@#TODO below 3 lines
        obj->configurePlugin(maxM, maxN,  maxK);
        // obj->getWorkspaceSize();
        obj->initialize();
        return obj;
    }
    catch (std::exception const& e)
...
// IPluginV2* BTWeightOnlyGroupwiseQuantMatmulPluginCreator::deserializePlugin(
...
//         auto* obj = new BTWeightOnlyGroupwiseQuantMatmulPlugin(serialData, serialLength, pluginProfiler);
