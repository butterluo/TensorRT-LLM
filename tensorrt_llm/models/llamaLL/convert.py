...
def load_weights_from_hf_model(hf_model,
    ...
    def convert_layer(l):
        ...
        if (l < config.num_hidden_layers - 1):#@#
            nxtprefix = f'model.layers.{l+1}.'
            input_ln_weight = get_weight(model_params, nxtprefix + 'input_layernorm',
                                        dtype)
        else:
            input_ln_weight = get_weight(model_params, 'model.norm', dtype)
    ...
    if hf_model.config.tie_word_embeddings:
        raise ValueError("tie_word_embeddings NOT support")
