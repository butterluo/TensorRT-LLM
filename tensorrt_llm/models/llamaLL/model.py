...
from ...layers import (MOE, Attention, AttentionMaskType, ColumnLinear,
                       Embedding, GatedMLP, PositionEmbeddingType, RmsNorm, KeyValueCacheParams)
...
from ..modeling_utils import (DecoderModelForCausalLM,
                              QuantConfig, check_share_embedding)
from .config import LLaMALLConfig
...
class DecoderLLLayerList(ModuleList):
    ...
    def forward(self,
        ...
        for layer_idx, (layer, past) in enumerate(
            ...
            if use_cache:
                presents.append(hidden_states[2])
...
class LLaMALLDecoderLayer(Module):

    def __init__(self, config: LLaMALLConfig, layer_idx: int):
        super().__init__()
        ...
        if self.has_residual_mlp:
            raise ValueError("has_residual_mlp NOT support in llamaLL")
...
class LLaMALLModel(Module):

    def __init__(self, config: LLaMALLConfig) -> None:
        super().__init__()
        ...
        self.layers = DecoderLLLayerList(LLaMALLDecoderLayer, config)
...
class LLaMALLForCausalLM(DecoderModelForCausalLM):
    config_class = LLaMALLConfig

    def __init__(self, config: LLaMALLConfig):
        transformer = LLaMALLModel(config)
    ...
    def from_hugging_face(
            ...
            **kwargs):
        ''' Create a LLaMALLForCausalLM object from give parameters
        '''
        import transformers
        ...
        config = LLaMALLConfig.from_hugging_face(hf_config_or_dir,
                                               dtype=dtype,
                                               mapping=mapping,
        ...
        model = LLaMALLForCausalLM(config)
        model.load(weights)
        return model
    ...
    def from_meta_ckpt(cls,
                       ...
                       quant_config: Optional[QuantConfig] = None,
                       **kwargs):
        config = LLaMALLConfig.from_meta_ckpt(meta_ckpt_dir,
                                            dtype=dtype,
                                            mapping=mapping,
                                            quant_config=quant_config,
                                            **kwargs)
    ...
    def quantize(
        ...
        **kwargs,
        ...
        config = LLaMALLConfig.from_hugging_face(hf_model_dir,
                                               dtype=dtype,
                                               mapping=mapping,
                                               quant_config=quant_config,
                                               **kwargs)
