...
def load_from_hf_llama(tensorrt_llm_llama: 'LLaMALLForCausalLM',
                       hf_llama,
                       mapping=Mapping(),
                       dtype='float32',
                       use_gemm_woq_plugin=True):                                         #@#MODIFY this function to adapt LLaMALL
    logger.info('Loading weights from HF LLaMA...')
    tik = time.time()
    ...
    for k, v in model_params.items():
        ...
        if 'model.embed_tokens.weight' in k:
            ...
            weights['transformer.layers.{}.input_layernorm.weight'.format(
                    hf_llama.config.num_hidden_layers - 1)] = v
        elif 'model.layers.0.input_layernorm.weight' in k:
            # if mapping.is_last_pp_rank():
            if mapping.is_first_pp_rank():
            ...
            if 'input_layernorm.weight' in k and idx > 0:
                weights['transformer.layers.{}.input_layernorm.weight'.format(
                    idx - 1)] = v
